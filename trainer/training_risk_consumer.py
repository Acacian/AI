...
TOPIC = "ai_risk_training_btcusdt_1m"
...
def train_step(batch_data):
    try:
        ...
        print(f"📉 Risk Loss: {loss.item():.6f} | Pred: {pred[0].item():.4f} Target: {y[0].item():.4f}")
...
def export_onnx():
    try:
        ...
        print(f"✅ ONNX Exported (Risk): {MODEL_PATH}")
    except Exception as e:
...
def main():
    consumer = KafkaConsumer(
        ...
        group_id="trainer-risk"
    )
    print(f"🛡️ Risk Consumer Started: {TOPIC}")
