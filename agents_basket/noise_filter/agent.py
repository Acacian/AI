...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s | NoiseFilter | %(levelname)s | %(message)s",
    datefmt="%Y-%m-%d %H:%M:%S",
...
class NoiseFilterAgent(BaseAgent):
    ...
    def run_online(self):
        ...
        logger.info(f"📡 NoiseFilter consuming from: {self.topic}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        logger.error("❌ 사용법: python -m agents_basket.noise_filter.agent <config_path>")
        sys.exit(1)

    config_path = sys.argv[1]
    agent = NoiseFilterAgent(config_path)
    agent.run()
