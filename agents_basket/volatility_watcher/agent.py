import os, sys, yaml, json, glob, torch
...
class Agent:
    ...
    def run_offline(self, data_dir="data"):
        print("📂 [Offline] VolatilityWatcher 오프라인 학습 시작", flush=True)
    ...
    def run(self):
        if self.should_pretrain():
            print("🧠 모델 없음 → 오프라인 선학습 먼저 수행", flush=True)
            self.run_offline()
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("❌ 사용법: python -m agents_basket.volatility_watcher.agent <config_path> [offline]", flush=True)
        sys.exit(1)
